...
Usage:
    python3 parse_junit.py \
        --results-dir  test-results \
        --summary-file "$GITHUB_STEP_SUMMARY" \
        --output-file  test-results/summary.json
"""
...
def parse_file(xml_path: str) -> Suite:
    ...
    suite = Suite(
        name=suite_elem.get("name", Path(xml_path).stem),
    ...
    for tc in suite_elem.findall(".//testcase"):
        ...
        suite.cases.append(TestCase(
            ...
            status=status,
            message=message.strip()[:400],   # cap length for readability
            stdout=(sysout.text or "").strip()[:200] if sysout is not None else "",
        ))
...
def write_summary(suites: List[Suite], out_fh) -> dict:
    ...
    for suite in suites:
        ...
        # Table of failing / erroring tests
...
def main():
    ...
    if not xml_files:
        ...
        # Write a neutral summary so the step doesn't hard-fail
