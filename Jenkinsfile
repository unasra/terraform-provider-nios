...
properties([
    parameters([
        string(name: 'GM_URL',     defaultValue: 'https://172.28.82.171', description: 'URL For the GRID Master'),
        string(name: 'MEMBER_URL', defaultValue: 'https://172.28.83.237', description: 'URL for the GRID Member'),
        ...
        string(name: 'BRANCH',     defaultValue: 'test_action_910',                   description: 'Branch name to checkout')
    ])
])
...
node('Cloud-test1-172.28.81.12-label') {
    timestamps {
        timeout(time: 240, unit: 'MINUTES') {
            ...
            env.JUNIT_GLOB       = 'test-results/**-junit.xml'
            ...
            try {
                ...
                stage('Checkout') {
                    ...
                    // Explicitly capture SHA — the simple `git` step does not
                    // reliably populate env.GIT_COMMIT in the same Groovy scope.
                    env.GIT_COMMIT = sh(script: 'git rev-parse HEAD', returnStdout: true).trim()
                    echo "Building commit: ${env.GIT_COMMIT}"

                    // Load signal credentials early so we can immediately signal job start
                    withCredentials([
                        string(
                            credentialsId: 'tf_ut_store_id',
                            variable: 'GIST_ID_VAL'
                        ),
                        string(
                            credentialsId: 'tf_ut_signal_token',
                            variable: 'SIGNAL_TOKEN_VAL'
                        )
                    ]) {
                        env.SIGNAL_GIST_ID = GIST_ID_VAL
                        env.SIGNAL_TOKEN   = SIGNAL_TOKEN_VAL
                    }

                    // Load the signal helper from the repo itself
                    signal = load '.github/scripts/signal.groovy'

                    // Notify GitHub Action that Jenkins has picked up this commit
                    signal.started()
                }
                ...
                def junitOutput = sh(script: 'find test-results -name "*-junit.xml" 2>/dev/null || true', returnStdout: true).trim()
                def junitFileMap = [:]
                if (junitOutput) {
                    junitOutput.split('\n').each { path ->
                        def fname = path.tokenize('/')[-1]
                        junitFileMap[fname] = readFile(path)
                    }
                }
                signal.success('tests-complete', [
                    message    : currentBuild.result == 'FAILURE' ? 'Tests completed with failures — check artifacts' : 'All tests passed',
                    junit_files: junitFileMap])
