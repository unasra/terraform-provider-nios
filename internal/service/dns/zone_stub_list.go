...
var _ list.ListResource = &ZoneStubList{}
var _ list.ListResourceWithConfigure = &ZoneStubList{}

func NewZoneStubList() list.ListResource {
	return &ZoneStubList{}
}

// ZoneStubList defines the List implementation.
type ZoneStubList struct {
	client *niosclient.APIClient
}

func (l *ZoneStubList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_zone_stub"
}

func (l *ZoneStubList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (l *ZoneStubList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Stub Zones.",
...
func (l *ZoneStubList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.ZoneStub, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list ZoneStub, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenZoneStub(ctx, &item, &result.Diagnostics)
